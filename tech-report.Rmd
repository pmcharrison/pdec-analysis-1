---
title: "Detection of structural regularities in rapid sound sequences: Computational modelling of memory constraints in auditory prediction"
author:
  ...
  - name: Marcus T. Pearce
    affiliation: Queen Mary University of London; Aarhus University
...
```{r, results = "asis"}
if (!knitr::is_html_output()) {
  cat("*Peter M. C. Harrison; Roberta Bianco; Maria Chait; Marcus T. Pearce*\n")
} 
```
...
The Prediction by Partial Match (PPM) algorithm [@Cleary1984],
...
in sequential data.
However, PPM lacks cognitive realism in two important ways:
a) its perfect memory, and b) its inflexible learning rates.
...
which defines how training data are weighted as a function of time elapsed.
...
We demonstrate the application of this model to recent data from 
sequence perception paradigms [e.g. @Barascud2016],
...
Our algorithm begins with PPM as formulated by @Pearce2005.
The original PPM algorithm makes predictions from learned 
...
```{r, cache = TRUE, fig.cap = "Four example decay functions. A) Null decay function, corresponding to the original PPM algorithm. B) As in A, but with a short-term learning rate of a = 0.75. C) As in B, but with a short-term memory half-life of c = 50. D) As in C, but with a long-term learning rate of d = 0.1."}
f <- list(decay_none, 
...
with an example decay function that better approximates human behaviour.
(short-term learning rate = 0.15, long-term learning rate = 0.0005,
short-term memory half-life = 50 tones).
In Block 1, we see that information content in the random phase is 
systematically lower and less variable.
...
```{r analysis1, cache = TRUE, fig.cap = "Example analysis comparing the original PPM model to a PPM decay model. See main text for details. The black lines plot mean values, and the blue ribbons plot standard deviations."}
dat %>%
